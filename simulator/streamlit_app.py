...
with right_col:
    ...
    if fire_btn:
        ...
        if len(class_indices) == 0:
            st.session_state["log_lines"].append(f"[{ts}] [X] ERROR - no samples found for class '{selected_class}'")
        else:
            idx          = random.choice(class_indices.tolist())
            raw_features = X_all[idx]

            # Build payload matching the FlowFeatures schema:
            #   { "features": list[float],   <- 115-element list (not flat dict)
            #     "source_ip": str,
            #     "dest_ip":   str,
            #     "src_port":  int,
            #     "dst_port":  int }
            payload = {
                "features":   [float(v) for v in raw_features],   # list[float]
            ...
            # Keep true_label only in session state for display -- NOT sent to API
            _true_label = selected_class
            ...
            st.session_state["log_lines"].append(f"[{ts}] > SRC  {payload['source_ip']}:{payload['src_port']}")
            st.session_state["log_lines"].append(f"[{ts}] > DST  {payload['dest_ip']}:{payload['dst_port']}")
            ...
            try:
                ...
                if response.status_code == 200:
                    st.session_state["log_lines"].append(f"[{ts}] [OK] HTTP 200 - PAYLOAD ACCEPTED")
                ...
                st.session_state["log_lines"].append(f"[{ts}] [X] CONNECTION REFUSED - backend offline at {FASTAPI_URL}")
                st.session_state["log_lines"].append(f"[{ts}]   -> Start backend: uvicorn backend.main:app --reload")
            except Exception as e:
                st.session_state["log_lines"].append(f"[{ts}] [X] ERROR - {str(e)}")
