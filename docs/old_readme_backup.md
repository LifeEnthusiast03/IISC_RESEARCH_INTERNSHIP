# ≡ƒ¢í∩╕Å Real-Time Incident Tracking & Autonomous Threat Remediation(Detailed Note)
...
- **Responds** autonomously using a Deep Q-Network (DQN) agent that selects the optimal containment action in milliseconds ΓÇö without human intervention
...
Replay Simulator (CICIDS2017 CSV rows sent as live HTTP POST)
       Γöé  POST /predict  (raw 115-feature flow + metadata)
       Γû╝
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé  FastAPI Backend    Γöé   Apply scaler.pkl ΓåÆ Autoencoder forward pass
Γöé  /predict endpoint  Γöé   Reconstruction error = ΓÇûx ΓêÆ x╠éΓÇû┬▓
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
          Γöé  error > threshold?
          Γö£ΓöÇΓöÇ NO  ΓåÆ Log benign, discard
          Γû╝  YES
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé  Stage 1            Γöé   Anomaly confirmed
Γöé  Autoencoder        Γöé   Trained ONLY on benign traffic
Γöé  (Anomaly Detector) Γöé   ΓåÆ High error = attack detected
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
          Γöé
          Γû╝
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé  Stage 2            Γöé   Selects from 5 actions:
Γöé  DQN Agent          Γöé   Block IP / Revoke creds /
Γöé  (Auto-Remediation) Γöé   Isolate server / Kill process / Monitor
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
          Γöé
          Γû╝
...
Γöé  Dummy Action Executor                              Γöé
Γöé  Simulated network state dict (IP ΓåÆ status)         Γöé
Γöé  No real firewall/router ΓÇö demo-safe                Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
          Γöé
          Γû╝
...
### ≡ƒôà 11 June 2026 ΓÇö Project Kickoff & Problem Understanding
...
**Concepts understood:**
- Reconstruction error as anomaly score: `L = ΓÇûx ΓêÆ x╠éΓÇû┬▓`
- Why training only on BENIGN rows makes the model generalize to unseen attacks

---

### ≡ƒôà 12 June 2026 ΓÇö Network Security Domain Knowledge
...
**Key insight:** CICIDS2017 has severe class imbalance (~80% BENIGN). The autoencoder approach is ideal because it trains only on the abundant normal class ΓÇö rarity of attacks becomes irrelevant.
...
- Heartbleed: port 443, small request ΓåÆ unusually large response

---

### ≡ƒôà 13 June 2026 ΓÇö Full Stack Application Architecture Design
...
```
Layer 1 ΓÇö Data Layer       : Pandas, NumPy, Scikit-learn, PostgreSQL
Layer 2 ΓÇö ML Layer         : PyTorch + CUDA, Autoencoder, DQN, Gymnasium
Layer 3 ΓÇö Backend Layer    : FastAPI, WebSockets, SQLAlchemy, Uvicorn
Layer 4 ΓÇö Frontend Layer   : React.js, Recharts, WebSocket client
```

**4 artefact files produced by training:**
- `autoencoder.pt` ΓÇö trained autoencoder weights
- `dqn_agent.pt` ΓÇö trained DQN agent weights
- `scaler.pkl` ΓÇö fitted MinMaxScaler parameters
- `threshold.json` ΓÇö anomaly detection threshold value

**Runtime inference pipeline (one network flow):**
```
Receive flow ΓåÆ Preprocess (scaler) ΓåÆ Autoencoder forward pass
ΓåÆ Compute reconstruction error ΓåÆ Compare to threshold
ΓåÆ [Normal] Log & discard
ΓåÆ [Anomaly] DQN selects action ΓåÆ Execute ΓåÆ Store in PostgreSQL ΓåÆ Push via WebSocket
```

---

### ≡ƒôà 14 June 2026 ΓÇö Python for Machine Learning

**Topics covered:**

**NumPy:**
- Arrays and shapes: `data.shape` ΓåÆ `(2273097, 78)`
...
- Stripping column names: `df.columns.str.strip()` ΓÇö critical for CICIDS2017
...
### ≡ƒôà 15 June 2026 ΓÇö Machine Learning Fundamentals
...
**2. Loss Functions**
- MSE (Mean Squared Error): `loss = ((x - x_hat)**2).mean()` ΓÇö used in autoencoder
- Binary Cross-Entropy: used for supervised classification
- Reward signal: `+10` threat neutralized, `-3` false alarm, `-5` service disrupted ΓÇö used in DQN

**3. Gradient Descent**
- Weights updated every mini-batch: `weight = weight - lr * gradient`
- In PyTorch: `loss.backward()` ΓåÆ `optimizer.step()` ΓåÆ `optimizer.zero_grad()`
...
- Test (15%): touch ONCE at the very end ΓÇö final performance measurement
- **Critical rule:** fit scaler only on training data, `.transform()` val and test

**5. Overfitting vs Underfitting**
- Underfitting: both train and val loss high ΓåÆ model too simple
- Overfitting: train loss low, val loss rising ΓåÆ model memorizing
...
- Precision = TP / (TP + FP) ΓåÆ how many alarms are real
- Recall = TP / (TP + FN) ΓåÆ how many real attacks are caught (most critical for security)
- F1 Score = 2 ├ù (P ├ù R) / (P + R) ΓåÆ primary comparison metric
- ROC-AUC ΓåÆ threshold-independent performance measure
- **Target metrics:** Recall > 0.90, Precision > 0.85, F1 > 0.87, ROC-AUC > 0.95

---

### ≡ƒôà 15 June 2026 ΓÇö Dataset Analysis
...
| dos_hulk.csv | 268,098 KB | Attack ΓÇö DoS Hulk |
| ddos_loit.csv | 88,972 KB | Attack ΓÇö DDoS LOIT |
| portscan.csv | 115,457 KB | Attack ΓÇö PortScan |
| botnet_ares.csv | 3,877 KB | Attack ΓÇö Botnet ARES |
| dos_golden_eye.csv | 7,846 KB | Attack ΓÇö DoS GoldenEye |
| dos_slowhttptest.csv | 5,137 KB | Attack ΓÇö DoS SlowHTTPTest |
| dos_slowloris.csv | 4,151 KB | Attack ΓÇö DoS Slowloris |
| ftp_patator.csv | 7,290 KB | Attack ΓÇö FTP-Patator |
| heartbleed.csv | 12 KB | Attack ΓÇö Heartbleed (very few rows) |
| ssh_patator_new.csv | 4,802 KB | Attack ΓÇö SSH-Patator |
| web_brute_force.csv | 1,874 KB | Attack ΓÇö Web Brute Force |
| web_sql_injection.csv | 21 KB | Attack ΓÇö SQL Injection |
| web_xss.csv | 913 KB | Attack ΓÇö XSS |

**UNSW-NB15 files downloaded from Kaggle:**

| File | Size | Notes |
|---|---|---|
| NUSW-NB15_features.csv | 4 KB | Feature descriptions ΓÇö read first |
...
- **Primary:** CICIDS2017 ΓÇö full training and evaluation pipeline
- **Secondary:** UNSW-NB15 ΓÇö cross-dataset generalization test (train on CICIDS, test on UNSW without retraining)
- Strategy A (separate models) chosen over combined training ΓÇö appropriate for beginner implementation

**Loading UNSW-NB15 correctly:**
```python
# UNSW-NB15 has NO header row ΓÇö must assign column names manually
...
### ≡ƒôà 15 June 2026 ΓÇö Project Charter & Planning
...
### ≡ƒôà 16 June 2026 ΓÇö Data Preprocessing Pipeline for CICIDS2017
...
#### Step 1 ΓÇö Load and combine all CSV files
...
#### Step 2 ΓÇö Fix column names ΓÜá∩╕Å CICIDS-specific gotcha
...
- Fix: `df.columns = df.columns.str.strip()` ΓÇö run immediately after loading
- Also drop any unnamed index columns: `df.loc[:, ~df.columns.str.contains('^Unnamed')]`

#### Step 3 ΓÇö Remove infinity values and NaN
...
- A single `inf` in a batch makes the entire loss `nan` ΓÇö training dies immediately
- Fix: `df.replace([np.inf, -np.inf], np.nan).dropna()`
- Typically removes ~2,000ΓÇô5,000 rows out of 1.3M ΓÇö acceptable loss

#### Step 4 ΓÇö Remove duplicate rows
- CICIDS2017 contains ~6% exact duplicate rows
- Duplicates bias what the model thinks is "normal" (some patterns over-represented)
- Fix: `df.drop_duplicates()` ΓÇö run AFTER step 3, not before
- Order matters: fix inf first, then deduplicate

#### Step 5 ΓÇö Fix data types
- PyTorch requires `float32` ΓÇö raw CICIDS data loads as `float64`
...
#### Step 6 ΓÇö Normalize features to 0ΓÇô1 range ΓÜá∩╕Å Most critical step
- CICIDS features span wildly different ranges: Flow Duration (0ΓÇô119,999,999) vs Header Length (0ΓÇô43,772)
- Without normalization, large-value features dominate; small features contribute nothing
- Use MinMaxScaler ΓÇö fit ONLY on training data, transform on val/test
...
#### Step 7 ΓÇö Split and save as .npy arrays
...
- Sanity checks before saving:
  - `np.isnan(X_train).any()` ΓåÆ must be `False`
  - `np.isinf(X_train).any()` ΓåÆ must be `False`
  - `X_train.min()` ΓåÆ `0.0`, `X_train.max()` ΓåÆ `1.0`
...
print(f"  Range:     {X_train.min():.3f} ΓÇô {X_train.max():.3f}")  # 0.0 ΓÇô 1.0
```

---

### ≡ƒôà 16 June 2026 ΓÇö Dataset Sufficiency Analysis

**Question answered: Is 1.81 GB enough to train the autoencoder and DQN?**

**Answer: Yes ΓÇö more than enough.**
...
| You have | ~1.3 million benign rows (26├ù minimum) |
...
**The one real concern ΓÇö class imbalance within attacks:**
- DoS Hulk: ~231,000 rows Γ£à excellent
- PortScan: ~159,000 rows Γ£à excellent
- DDoS: ~42,000 rows Γ£à good
- Web XSS: ~2,000 rows ΓÜá∩╕Å limited
- Heartbleed: ~11 rows Γ¥î statistically meaningless
...
- One epoch over 1M rows at batch_size=256: ~2ΓÇô5 minutes
- 50 epochs total: ~1.5ΓÇô4 hours
- Without GPU (CPU only): multiply by 10ΓÇô20├ù

**Memory usage after preprocessing:**
- 1.3M rows ├ù 78 features ├ù 4 bytes (float32) = ~390 MB RAM
- Fits comfortably in 8 GB RAM

---

### ≡ƒôà 16 June 2026 ΓÇö Backpropagation in Autoencoders

**Question answered: Does the autoencoder use backpropagation for training?**

**Answer: Yes ΓÇö exactly the same as any neural network.**

**The 4-step training loop (runs once per batch):**

| Step | What happens | PyTorch call |
|---|---|---|
| 1. Forward pass | Input X flows through encoder ΓåÆ bottleneck ΓåÆ decoder ΓåÆ produces reconstruction X╠é | `x_hat = autoencoder(x)` |
| 2. Compute loss | MSE between original and reconstruction: L = ΓÇûX ΓêÆ X╠éΓÇû┬▓ | `loss = criterion(x_hat, x)` |
| 3. Backward pass | PyTorch computes ΓêéL/Γêéw for every weight in every layer | `loss.backward()` |
| 4. Weight update | Every weight nudged to reduce loss: w = w ΓêÆ lr ├ù gradient | `optimizer.step()` |

**Key difference from a classifier:**
- Classifier: compares prediction vs external label
- Autoencoder: compares reconstruction X╠é vs original input X ΓÇö **the input IS the target**
- No labels needed. `TensorDataset(X_tensor, X_tensor)` ΓÇö same array for both input and target

**What backpropagation does in the autoencoder specifically:**
- Loss starts at the output (reconstruction error)
- Gradients flow BACKWARDS: output ΓåÆ decoder ΓåÆ bottleneck ΓåÆ encoder ΓåÆ input
...
- After thousands of batches of BENIGN traffic, all weights adjust to reconstruct normal patterns very well ΓåÆ low loss
- When an ATTACK flow arrives at inference, the weights have never optimized for it ΓåÆ high reconstruction error
...
# Wrap data ΓÇö input and target are the SAME array
...
### ≡ƒôà 16 June 2026 ΓÇö Model Size Analysis
...
- File size = (total parameters ├ù 4 bytes) + small PyTorch metadata overhead

**Autoencoder parameter breakdown (115 ΓåÆ 64 ΓåÆ 32 ΓåÆ 16 ΓåÆ 32 ΓåÆ 64 ΓåÆ 115):**

| Layer | Dimensions | Weights | Biases | Total Params |
|---|---|---|---|---|
| Encoder Layer 1 | 115 ΓåÆ 64 | 7,360 | 64 | 7,424 |
| Encoder Layer 2 | 64 ΓåÆ 32 | 2,048 | 32 | 2,080 |
| Bottleneck | 32 ΓåÆ 16 | 512 | 16 | 528 |
| Decoder Layer 1 | 16 ΓåÆ 32 | 512 | 32 | 544 |
| Decoder Layer 2 | 32 ΓåÆ 64 | 2,048 | 64 | 2,112 |
| Output Layer | 64 ΓåÆ 115 | 7,360 | 115 | 7,475 |
| **TOTAL** | ΓÇö | **19,840** | **323** | **20,163** |

**DQN agent parameter breakdown (115 ΓåÆ 128 ΓåÆ 64 ΓåÆ 5):**

| Layer | Dimensions | Total Params |
|---|---|---|
| Hidden Layer 1 | 115 ΓåÆ 128 | 14,848 |
| Hidden Layer 2 | 128 ΓåÆ 64 | 8,256 |
| Output Layer | 64 ΓåÆ 5 | 325 |
| **TOTAL** | ΓÇö | **23,429** |
...
|---|---|---|---|
| autoencoder.pt | 20,163 | ~85ΓÇô100 KB | Smaller than a JPEG image |
| dqn_agent.pt | 23,429 | ~95ΓÇô115 KB | Very lightweight |
| scaler.pkl | 115 ├ù 2 values | ~7 KB | min/max per feature |
| threshold.json | 1 value | < 1 KB | e.g. `{"threshold": 0.045}` |
| **Total artefacts** | ΓÇö | **~200 KB** | Everything FastAPI loads at startup |

**Key insight:** The entire production ML system ΓÇö both models, the scaler, and the threshold ΓÇö fits in ~165 KB total. FastAPI loads all of this in under 1 second at startup.

**Comparison with other models:**
- Your autoencoder: ~90 KB
- ResNet-50 (image classifier): 98 MB (1,090├ù larger)
- BERT Base (NLP): 440 MB (4,900├ù larger)
- GPT-2 Small: 548 MB (6,100├ù larger)
...
### ≡ƒôà 17 June 2026 ΓÇö Split Preprocessing Pipeline (Benign + Attack)
...
#### `preprocess_benign.py` ΓÇö Autoencoder Training Data
- Reads from `data/cicids2017/benign_data/` (5 files: monday ΓåÆ friday)
- Applies full CICIDS2017 cleaning pipeline:
  1. Strip column name whitespace (CICIDS2017-specific gotcha: `' Label'` Γëá `'Label'`)
  2. Drop identifier columns (`flow_id`, `timestamp`, `src_ip`, `dst_ip`)
  3. Replace `inf` / `-inf` ΓåÆ `NaN`, then drop NaN rows
...
- **Fits** `MinMaxScaler` on training split ONLY ΓÇö the scaler never sees val/test/attack data
- Splits 70% train | 15% val | 15% test (all benign ΓÇö no stratification needed)
...
#### `preprocess_attack.py` ΓÇö DQN Agent Environment Data
...
- Encodes attack type strings ΓåÆ integers via `LabelEncoder`
- **Loads** (never re-fits) `scaler.pkl` produced by `preprocess_benign.py`
  ...
  - Attack features can legitimately fall slightly outside `[0, 1]` after this transform ΓÇö that is expected and correct
...
- The autoencoder uses `sigmoid` activation in the output layer ΓåÆ output spans `[0, 1]`
- Input features must also be in `[0, 1]` for the reconstruction loss to be meaningful
- `StandardScaler` produces z-scores (can be negative, unbounded) ΓÇö wrong for this use case

**Data leakage rule confirmed:**
```
SCALER FIT  ΓåÆ only X_train (benign)
SCALER TRANSFORM ΓåÆ X_val, X_test, X_attacks (no fit, no leakage)
```

**`preprocess.py` removed** ΓÇö superseded by the two split scripts above. Using a single combined script would have contaminated the autoencoder training set and used the wrong scaler type.

---

### ≡ƒôà 18 June 2026 ΓÇö Autoencoder Training Script Implementation
...
#### Model Architecture ΓÇö `Autoencoder` (nn.Module)

| Component | Layers | Activation |
|---|---|---|
| Encoder | 115 ΓåÆ 64 ΓåÆ 32 ΓåÆ 16 | ReLU + Dropout(0.2) after each hidden layer |
| Bottleneck | 16 dimensions | ReLU |
| Decoder | 16 ΓåÆ 32 ΓåÆ 64 ΓåÆ 115 | ReLU (hidden), **Sigmoid** (output) |
...
# 95th-percentile ΓåÆ anomaly threshold
...
| `threshold.json` | `{"threshold": <float>}` ΓÇö 95th-percentile val MSE |
| `training_history.json` | `{"train_loss": [...], "val_loss": [...]}` ΓÇö one float per epoch |

**Code style decisions:**
- Path anchoring via `os.path.dirname(os.path.abspath(__file__))` ΓåÆ project root (matches `preprocess_benign.py`)
...
### ≡ƒôà 18 June 2026 ΓÇö Frontend Dashboard Initiation (ThreatSentinel UI)

**Topics covered:**
- Scaffolded the React/TypeScript incident dashboard using Vite
- Integrated Tailwind CSS v4 with the `@tailwindcss/vite` plugin
- Built the initiation UI (`ThreatSentinel`) ΓÇö a dark cybersecurity dashboard scaffold
...
**Tailwind v4 wiring (Vite plugin approach ΓÇö no `tailwind.config.js` needed):**
```typescript
// vite.config.ts
...
| **Pipeline tracker** | 5-stage progress list (Data Ingestion ΓåÆ Live Inference) with status dots |
| **Live threat feed** | Tabular alert list + WebSocket connection status + ΓÇ£awaiting modelΓÇ¥ banner |
...
### ≡ƒôà 18 June 2026 ΓÇö Replay Simulator Design (Demo Infrastructure)
...
Replay Simulator
    Γöé
    Γöé  1. Load a pool of rows from raw CICIDS2017 CSVs at startup
    Γöé     (mix of benign + attack, keeping src_ip/dst_ip/true_label locally)
    Γöé
    Γöé  2. Every N seconds (configurable), pick one row at random
    Γöé     and POST its 115 feature values (unscaled) to /predict
    Γöé
    Γöé  3. Print ground-truth vs prediction comparison locally
    Γöé     (running precision/recall sanity check during demo)
    Γû╝
FastAPI /predict
```
...
| Decision | Rationale |
|---|---|
| Read from raw CSVs, not `.npy` | `.npy` files strip `src_ip`, `dst_ip`, true label ΓÇö needed for display and sanity checking |
...
| True label stays **client-side** | Never sent to backend ΓÇö preserves fair evaluation; used for live precision/recall printing |
| Sample a fixed pool at startup | Avoids re-reading 1.8 GB of CSVs per request; same cost as preprocessing |
| `time.sleep(random 0.2ΓÇô2s)` | Makes flow arrival feel live; configurable via `--rate` flag |
| `--benign-ratio` flag | Controls attack frequency ΓÇö lower for dramatic demo, higher for realistic baseline |
...
- Dashboard displays per-IP status badges live: ≡ƒƒó active / ≡ƒö┤ blocked / ≡ƒƒí isolated / ≡ƒö╡ monitored

**5 remediation actions the DQN can select:**

| Action | Simulated effect |
|---|---|
| Block IP | Source IP status ΓåÆ `blocked` |
| Revoke credentials | Session status ΓåÆ `revoked` |
| Isolate server | Destination status ΓåÆ `isolated` |
| Kill process | Process status ΓåÆ `terminated` |
| Monitor (no action) | No state change; incident logged only |

**New `simulator/` module added to project structure:**
- `simulator/replay_simulator.py` ΓÇö main script (to be written next)
- `simulator/README.md` ΓÇö usage instructions (to be written next)

**Status:** Folder scaffold created. Code to be written in the next session.

---

### ≡ƒôà 19 June 2026 ΓÇö End-to-End Autoencoder Evaluation & Hybrid Architecture Decision
...
**Topics covered:**
- Trained the autoencoder (v1) with the original 115ΓåÆ64ΓåÆ32ΓåÆ16ΓåÆ32ΓåÆ64ΓåÆ115 architecture; got F1=0.6967 on initial local test, TPR=0.56 (well below the >0.90 recall target from the project charter).
- Diagnosed the gap via threshold sweeping (`eval_threshold.py`) using `precision_recall_curve` on the full benign val set + full attack set. Found the naive 95th-percentile threshold was too conservative, but also found that a "best F1" threshold from the skewed dataset ratio was operationally useless (FPR=0.88 ΓÇö would flag most benign traffic).
- Built `tests/test_autoencoder.py` with a realistic percentile-based threshold sweep (based on benign-only error percentiles); selected p92.5 (threshold=0.00071623) as the practical operating point ΓÇö Recall=0.7145, Precision=0.9050, F1=0.7985, FPR=0.075.
- Built `training/analyze_feature_errors.py` to diagnose WHY 6 specific attack classes (SSH-Patator, Web_Brute_Force, Web_XSS, Web_SQL_Injection, FTP-Patator, Botnet_ARES) were poorly detected. Found they split into two groups:
  ...
  - **SSH-Patator, Web_Brute_Force, Web_XSS, Web_SQL_Injection** had 0-1 significant features ΓÇö i.e. genuinely indistinguishable from benign at the flow-feature level.
- Ran an ablation: trained autoencoder v2 with a wider bottleneck (115ΓåÆ80ΓåÆ48ΓåÆ24ΓåÆ48ΓåÆ80ΓåÆ115, up from 16-dim) to test whether more bottleneck capacity would recover FTP-Patator/Botnet_ARES detection.
- Compared v1 vs v2 (`training/compare_models.py`): v2 won on aggregate (F1 0.7136ΓåÆ0.8217, Recall 0.58ΓåÆ0.71, same FPR=0.05) and dramatically improved Port_Scan (0.51ΓåÆ0.99 TPR) as an unexpected bonus, but did NOT recover FTP-Patator (0.2658ΓåÆ0.2656, flat) or Botnet_ARES (0.0372ΓåÆ0.0369, flat) ΓÇö confirming these two classes hit a feature-representation ceiling, not a model-capacity ceiling.
...
**Key decisions made:**
- Confirmed the reconstruction-error architecture has a real ceiling for 5 attack types (FTP-Patator, Botnet_ARES, SSH-Patator, Web_Brute_Force, Web_XSS) ΓÇö these attacks statistically resemble benign traffic at the 115-feature flow level, and no amount of threshold tuning or bottleneck widening recovers them.
...
**Next Steps:**
- Build `training/build_hybrid_dataset.py` ΓÇö construct a labeled dataset of the 5 weak attack classes + a matched benign sample, stratified train/val/test split.
- Build `training/train_hybrid_classifier.py` ΓÇö train a class-weighted multi-class XGBoost classifier on this dataset.
- Build `training/evaluate_combined_pipeline.py` ΓÇö evaluate the FULL two-stage pipeline (autoencoder v2 + hybrid classifier together) end to end on the full attack/benign test data, comparing combined metrics against the autoencoder-alone baseline.
...
### ≡ƒôà 22 June 2026 ΓÇö Deep Dive into Preprocessing and Autoencoder Model
...
### ≡ƒôà 23 June 2026 ΓÇö DoS_Hulk False-Negative Diagnosis & Hybrid Classifier Extension to 7 Classes

**Context:** After completing `evaluate_combined_pipeline.py`, DoS_Hulk was identified as the dominant source of false negatives in the combined two-stage pipeline: 297,642 total samples, TPR = 0.5194, 143,056 flows missed ΓÇö accounting for ~95% of all remaining false negatives.

---

#### Part 1 ΓÇö Root-Cause Diagnosis (`training/diagnose_dos_hulk.py`)

Built a 4-step diagnostic script to answer: **should we lower the AE threshold, or extend the hybrid classifier?**

**STEP 1 ΓÇö Error distribution analysis:**
- Loaded `errors_attack.npy` + `y_attacks_str.npy`, filtered to DoS_Hulk
- Key finding: threshold sits at the **48th percentile** of DoS_Hulk errors ΓÇö nearly half the class has very low reconstruction error
- Missed-flow bucket split:

| Bucket | Count | % of missed |
|---|---|---|
| Near threshold `[0.5├ùT, T]` | 4,414 | 3.09% |
| Far below `[0, 0.5├ùT)` | 138,642 | 96.91% |

- **Verdict: LIKELY REQUIRES FEATURE/MODEL FIX, NOT THRESHOLD** ΓÇö 96.9% of missed DoS_Hulk flows sit far below even half the threshold, meaning they are nearly indistinguishable from benign traffic as far as the autoencoder is concerned. Lowering the threshold enough to catch them would flood the pipeline with false positives.

**STEP 2 ΓÇö Threshold sensitivity sweep (always runs):**

| Threshold | Hulk TPR | Overall TPR | Benign FPR | Note |
|---|---|---|---|---|
| 0.0009142810 | 0.5194 | 0.7121 | 0.0500 | CURRENT |
| 0.0007 | 0.5286 | 0.7171 | 0.0633 | FPRΓëñ0.10 |
| 0.0005 | 0.5340 | 0.7211 | 0.0944 | FPRΓëñ0.10 |
| 0.0003 | 0.5504 | 0.7415 | 0.1483 | FPR>0.10 ΓÜá∩╕Å |
| 0.0002 | 0.5802 | 0.7597 | 0.2077 | FPR>0.10 ΓÜá∩╕Å |
| 0.0001 | 0.5802 | 0.7690 | 0.3020 | FPR>0.10 ΓÜá∩╕Å |

- Confirmed quantitatively: the maximum Hulk TPR gain within the FPR Γëñ 0.10 budget is only **+3.4 pp** (0.5194 ΓåÆ 0.5340). The 48% of missed flows that sit far below threshold cannot be recovered by any operationally realistic threshold change.

**STEP 3 ΓÇö Sub-population feature analysis (ran because verdict = FEATURE/MODEL FIX):**
...
- Top 15 features by absolute mean difference (`|╬ö|`) revealed clear structural separation
- **Separation ratio = avg top-15 |╬ö| / avg feature mean ΓëÑ 0.20** ΓåÆ classified as **CLEAR FEATURE SEPARATION**
- Interpretation: missed DoS_Hulk flows exhibit systematically different feature values from detected flows ΓÇö they represent a **distinguishable sub-variant** of DoS_Hulk that the autoencoder reconstructs accurately (hence low error), making them invisible to Stage 1.

**STEP 4 ΓÇö Recommendation issued:**
...
#### Part 2 ΓÇö Hybrid Classifier Extension: Adding DoS_Hulk as 7th Class
...
- Detected DoS_Hulk flows (already caught by AE) must NOT be included ΓÇö they would be double-counted
- Class imbalance (143K DoS_Hulk vs ~950 Web_XSS) handled downstream by `compute_sample_weight('balanced')` in `train_hybrid_classifier.py` ΓÇö no code change needed there

**Key implementation insight ΓÇö AE-error filtering:**
The filter `y_all == 'DoS_Hulk' AND errors_all Γëñ threshold` is the critical correctness constraint. It ensures only the flows the autoencoder **currently passes** as normal are trained as positive DoS_Hulk examples. Using all DoS_Hulk rows would mix in the already-detected flows and create training data that doesn't match the inference-time distribution.
...
| New `step1b_load_dos_hulk_missed()` | Reads threshold.json, masks DoS_Hulk AND error Γëñ threshold, returns only missed rows |
...
**`train_hybrid_classifier.py` ΓÇö zero changes:** dynamically reads `n_classes` from `hybrid_label_map.json`, so it automatically handles 7 classes.

**`evaluate_combined_pipeline.py` ΓÇö zero changes:** already handles any number of hybrid classes.
...
| **DoS_Hulk (missed)** | **143,056** | **X_attacks.npy filtered by errors_attack.npy Γëñ threshold** |
| **Total** | **~193,134** | |

**Pipeline fix resolved today:** XGBoost 3.x API breaking change ΓÇö `early_stopping_rounds` was moved from `fit()` to the `XGBClassifier` constructor. Fixed by moving the parameter into `XGB_PARAMS` dict.

**Diagnostics fix resolved today:** Float key precision mismatch in `diagnose_dos_hulk.py` ΓÇö `THRESHOLD_CANDIDATES` hardcoded `0.0009142810` (literal) but `json.load()` returns `0.0009142810013145208` (full precision), causing `KeyError` on dict lookup. Fixed by building the sweep list at runtime, prepending `current_threshold` (the exact JSON-loaded value) as the first element.

**Scripts executed today (in order):**
1. `python training/build_hybrid_dataset.py` ΓÇö rebuilt 7-class dataset (~193K rows)
2. `python training/train_hybrid_classifier.py` ΓÇö retrained XGBoost with 7 classes
3. `python training/evaluate_combined_pipeline.py` ΓÇö full two-stage pipeline evaluation
...
- `training/diagnose_dos_hulk.py` ΓÇö 4-step root-cause analysis for DoS_Hulk false negatives

**Files modified today:**
- `training/build_hybrid_dataset.py` ΓÇö added Step 1B (DoS_Hulk missed flow extraction), extended to 7-class output
- `training/train_hybrid_classifier.py` ΓÇö fixed XGBoost 3.x `early_stopping_rounds` API change
- `training/diagnose_dos_hulk.py` ΓÇö fixed float key precision bug in threshold sweep dict lookup

---

### ≡ƒôà 23 June 2026 ΓÇö Architecture Extension ΓÇö Attack-Type Classification + DQN Remediation Pipeline Design
...
The hybrid classifier (Stage 1B) successfully recovers the 5 attack types (Botnet_ARES, FTP-Patator, SSH-Patator, Web_Brute_Force, Web_XSS) that produce benign-like flow statistics and evade the autoencoder. Combined pipeline TPR is now 0.7497 vs 0.7121 for the autoencoder alone ΓÇö a confirmed +3.76 pp gain.

#### DoS_Hulk Diagnostic Finding

DoS_Hulk remains the dominant source of false negatives (297,642 total samples, TPR Γëê 0.52 autoencoder-alone). Deep diagnostic analysis via `training/diagnose_dos_hulk.py` revealed:

- **48% of DoS_Hulk flows form a distinguishable low-signal sub-variant**: characterised by short/single-packet flows and near-zero backward traffic features. These flows produce reconstruction errors far below even half the anomaly threshold ΓÇö they are structurally distinct from the high-signal DoS_Hulk flows the autoencoder does catch.
- **Threshold lowering cannot recover them** within any operationally realistic FPR budget: the maximum TPR gain within FPR Γëñ 0.10 is only +3.4 pp (0.5194 ΓåÆ 0.5340), while 96.9% of missed flows sit below 0.5├ù the threshold.
...
|---|---|---|---|
| 1A | Autoencoder (v2, 24-dim) | Unsupervised anomaly detection on flow reconstruction error | Γ£à Done |
| 1B | Hybrid XGBoost Classifier | Catches 5 weak attack classes that evade the AE | Γ£à Done |
| 2 | **Attack-Type Neural Network** | Broad multi-class classifier across all 13 CICIDS2017 attack types ΓÇö answers "which attack type is this flow?" | ≡ƒö¿ Building today |
| 3 | **DQN Remediation Agent** | Given flow features + attack-type prediction, selects optimal remediation action from 5 choices | ≡ƒö¿ Building today |
| 4 | LLM Agent Layer *(planned)* | For low-confidence / unknown attacks: generate human-readable summary, route to human review | ≡ƒôï Future work |

**Why an Attack-Type NN in addition to the hybrid classifier?**
The hybrid classifier (Stage 1B) covers only the 5 attack types with AE-evasion characteristics. The new Attack-Type NN is deliberately **broader** ΓÇö it classifies across ALL attack types present in CICIDS2017 ΓÇö so that the DQN agent has a rich, calibrated probability vector over attack types as part of its state, rather than only a 5-class or binary signal. This gives the DQN much better information for choosing the correct remediation action (e.g., distinguishing Block IP for DoS vs. Revoke Credentials for FTP-Patator).

**Key design constraint for the Attack-Type NN:** Heartbleed (n=12) and Web_SQL_Injection (n=24) are excluded from training due to insufficient samples. These two classes are documented as always "unclassified" by this NN ΓÇö they remain covered by the autoencoder's reconstruction error signal.

#### LLM Agent Layer ΓÇö Planned Future Work (Explicitly Out of Scope Today)
...
- **Scope:** advisory and human-facing ONLY ΓÇö the LLM layer will generate human-readable incident summaries and routing recommendations, and will NOT autonomously trigger any remediation actions.
...
### ≡ƒôà 24 June 2026 ΓÇö DQN Confusion Diagnosis ΓÇö Hulk/FTP Robustness Confirmed, Web Attack Action Merge

**Topics covered:**

- Diagnosed two confusion patterns identified in the Attack-Type NN's held-out test confusion matrix:
  - **(a) DoS_Hulk ΓåÆ FTP-Patator:** 3,363 of 44,647 true DoS_Hulk test rows were misclassified as FTP-Patator (7.5% error rate on that class)
  - **(b) Web_Brute_Force Γåö Web_XSS:** 187 of 410 Web_Brute_Force rows predicted as Web_XSS (45.6%), and 104 of 204 Web_XSS rows predicted as Web_Brute_Force (51.0%) ΓÇö near-random mutual confusion

- Built `evaluation/diagnose_nn_confusions.py` ΓÇö initial diagnostic covering both confusion pairs:
  - For each pair: computes normalized separation scores (`|mean_A ΓêÆ mean_B| / pooled_std`) across all 115 features and the 8 previously-identified "quiet sub-variant" features, to distinguish fixable model/training issues from genuine feature-representation ceilings
...
| Web_Brute_Force accuracy | ΓÇö | **99.51%** (408/410 ΓåÆ Revoke Credentials) |
| Web_XSS accuracy | ΓÇö | **98.04%** (200/204 ΓåÆ Revoke Credentials) |

Both classes now achieve high accuracy against the same target action. The overall pipeline improvement (0.9901 ΓåÆ 0.9922) reflects the removal of the conflicting reward signal that the DQN previously received for Web_XSS rows.

**Next steps:**

- **Identified architecture gap:** the Attack-Type NN has no mechanism to express "I don't recognize this attack type." It always outputs a prediction across its 11 trained classes, even for flows from attack types it was never trained on (Heartbleed n=12, Web_SQL_Injection n=24 ΓÇö both excluded from training due to insufficient samples) or any future zero-day attack type. In these cases the NN outputs an arbitrary softmax distribution, and the DQN acts on a meaningless signal.
...
Γöé                         OFFLINE TRAINING                             Γöé
Γöé                                                                      Γöé
Γöé  CICIDS2017 CSVs ΓöÇΓöÇΓû║ Preprocessing ΓöÇΓöÇΓû║ Autoencoder (v2, 24-dim)     Γöé
Γöé  UNSW-NB15 CSVs      (Pandas/NumPy)    (PyTorch+CUDA)               Γöé
Γöé                            Γöé                  Γöé                      Γöé
Γöé                       scaler.pkl         autoencoder.pt             Γöé
Γöé                       threshold.json     hybrid_classifier.pkl      Γöé
Γöé                       attack_type_nn.pt  dqn_agent.pt               Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
                            Γöé load at startup
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé                         ONLINE PRODUCTION (4-Stage Pipeline)         Γöé
Γöé                                                                      Γöé
Γöé  Network Flow                                                        Γöé
Γöé       Γöé                                                              Γöé
Γöé       Γû╝                                                              Γöé
Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                Γöé
Γöé  Γöé  Stage 1A: Autoencoder (v2)     Γöé  Reconstruction error > T?     Γöé
Γöé  Γöé  + Stage 1B: Hybrid Classifier  Γöé  ΓåÆ YES ΓåÆ anomaly confirmed     Γöé
Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ  ΓåÆ NO  ΓåÆ log benign, discard   Γöé
Γöé                   Γöé anomaly confirmed                                Γöé
Γöé                   Γû╝                                                  Γöé
Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                Γöé
Γöé  Γöé  Stage 2: Attack-Type NN        Γöé  Which of 13 CICIDS2017        Γöé
Γöé  Γöé  (115ΓåÆ128ΓåÆ64ΓåÆN, softmax)        Γöé  attack types is this flow?    Γöé
Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ  (+ confidence score)          Γöé
Γöé                   Γöé attack type + probs                              Γöé
Γöé                   Γû╝                                                  Γöé
Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                Γöé
Γöé  Γöé  Stage 3: DQN Agent             Γöé  State = [115 features |       Γöé
Γöé  Γöé  (state_dimΓåÆ128ΓåÆ64ΓåÆ5)           Γöé    AE error | attack-type probsΓöé
Γöé  Γöé                                 Γöé    | confidence]               Γöé
Γöé  Γöé  Actions: Block IP /            Γöé  ΓåÆ select optimal action       Γöé
Γöé  Γöé  Revoke Creds / Isolate Server /Γöé                                Γöé
Γöé  Γöé  Kill Process / Monitor         Γöé                                Γöé
Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                Γöé
Γöé                   Γöé                                                  Γöé
Γöé                   Γö£ΓöÇΓöÇ [high confidence] ΓåÆ Execute action directly   Γöé
Γöé                   Γöé                                                  Γöé
Γöé                   ΓööΓöÇΓöÇ [low confidence / future: Stage 4]            Γöé
Γöé                       LLM Agent Layer (planned ΓÇö NOT built yet)      Γöé
Γöé                       ΓåÆ human-readable summary ΓåÆ analyst review      Γöé
Γöé                                                                      Γöé
Γöé  ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ         Γöé
Γöé  WebSocket broadcast ΓåÆ React Dashboard ΓùäΓöÇΓöÇΓöÇΓöÇ PostgreSQL             Γöé
...
- [x] AI Opportunity Identification & Feasibility *(11ΓÇô13 Jun 2026)*
- [x] AI Project Planning & Architecture Design *(13ΓÇô15 Jun 2026)*
- [ ] Data Readiness & Engineering *(15ΓÇô21 Jun 2026)*
- [ ] AI Model Prototyping *(21ΓÇô29 Jun 2026)*
- [ ] Model Development & Optimization *(29 JunΓÇô08 Jul 2026)*
- [ ] AI Solution Integration & Deployment *(08ΓÇô17 Jul 2026)*
- [ ] AI Validation, Governance & Release Readiness *(17ΓÇô22 Jul 2026)*
- [ ] Production Monitoring & Continuous Improvement *(22ΓÇô24 Jul 2026)*
- [ ] Project Closure & Handover *(24ΓÇô25 Jul 2026)*
...
1. Sharafaldin, I., Lashkari, A. H., & Ghorbani, A. A. (2018). *Toward Generating a New Intrusion Detection Dataset and Intrusion Traffic Characterization*. ICISSP 2018.
   - ≡ƒöù https://www.semanticscholar.org/paper/Toward-Generating-a-New-Intrusion-Detection-Dataset-Sharafaldin-Lashkari/a27089efabc5f4abd5ddf2be2a409bff41f31199

2. Moustafa, N., & Slay, J. (2015). *UNSW-NB15: A Comprehensive Data Set for Network Intrusion Detection Systems*. MilCIS 2015.
   - ≡ƒöù https://ieeexplore.ieee.org/document/7348942

### Autoencoder & Anomaly Detection
3. *Analysis of Autoencoders for Network Intrusion Detection* (2021). PMC/NCBI.
   - ≡ƒöù https://pmc.ncbi.nlm.nih.gov/articles/PMC8272075/
...
4. Abdalla, A. et al. (2023). *Enhancing Network Intrusion Detection Using Attention-Based Deep Autoencoders*. Expert Systems with Applications, 213, 119102.
   - ≡ƒöù https://www.sciencedirect.com/science/article/pii/S0957417422019911

5. *Unsupervised Machine Learning Methods for Anomaly Detection in Network Packets* (2025). Electronics, MDPI.
   - ≡ƒöù https://www.mdpi.com/2079-9292/14/14/2779
...
6. *Feature Importance Guided Autoencoder for Dimensionality Reduction in Intrusion Detection Systems* (2026). Scientific Reports.
   - ≡ƒöù https://www.nature.com/articles/s41598-026-36695-9

7. *Deep learning-driven methods for network-based intrusion detection systems: A systematic review* (2025). ScienceDirect.
   - ≡ƒöù https://www.sciencedirect.com/science/article/pii/S2405959525000050

8. *A deep learning/machine learning approach for anomaly based network intrusion detection* (2025). Frontiers in AI.
   - ≡ƒöù https://www.frontiersin.org/journals/artificial-intelligence/articles/10.3389/frai.2025.1625891/full

### Deep Q-Network (DQN)
9. Mnih, V., Kavukcuoglu, K., Silver, D., et al. (2013). *Playing Atari with Deep Reinforcement Learning*. arXiv:1312.5602.
   - ≡ƒöù https://arxiv.org/abs/1312.5602

10. *Deep Q-Learning based Reinforcement Learning Approach for Network Intrusion Detection*. arXiv:2111.13978.
    - ≡ƒöù https://arxiv.org/pdf/2111.13978

### Reinforcement Learning for Cyber Defense
11. Nguyen, T. T., & Reddi, V. J. (2023). *Deep Reinforcement Learning for Cyber Security*. IEEE Transactions on Neural Networks and Learning Systems, 34(8), 3779ΓÇô3795.
    - ≡ƒöù https://research.monash.edu/en/publications/deep-reinforcement-learning-for-cyber-security/

12. *Deep Reinforcement Learning for Autonomous Cyber Operations: A Survey* (2024). arXiv:2310.07745.
    - ≡ƒöù https://arxiv.org/html/2310.07745v2
...
13. Ma, Y. et al. (2024). *Application of deep reinforcement learning algorithms for automatic threat detection and response in dynamic network environments*. SAGE Journals.
    - ≡ƒöù https://journals.sagepub.com/doi/abs/10.1177/14727978241309550

14. Sewak, M., Sahay, S. K., & Rathore, H. (2023). *Deep Reinforcement Learning in the Advanced Cybersecurity Threat Detection and Protection*. Information Systems Frontiers, 25(2), 589ΓÇô611.
    - ≡ƒöù https://link.springer.com/article/10.1007/s10796-022-10333-x
...
project/
Γö£ΓöÇΓöÇ data/
Γöé   Γö£ΓöÇΓöÇ cicids2017/
Γöé   Γöé   Γö£ΓöÇΓöÇ benign_data/         # 5 benign CSVs (monday ΓåÆ friday)
Γöé   Γöé   ΓööΓöÇΓöÇ attack_data/         # 13 attack CSVs (dos_hulk, portscan, etc.)
Γöé   Γö£ΓöÇΓöÇ unsw_nb15/               # raw UNSW-NB15 CSV files
Γöé   ΓööΓöÇΓöÇ processed/               # output of preprocessing scripts
Γöé       Γö£ΓöÇΓöÇ X_train_benign.npy   # autoencoder training input
Γöé       Γö£ΓöÇΓöÇ X_val_benign.npy     # autoencoder validation input
Γöé       Γö£ΓöÇΓöÇ X_test_benign.npy    # autoencoder test input
Γöé       Γö£ΓöÇΓöÇ X_attacks.npy        # all attack flows (600,141 ├ù 115, MinMax-scaled)
Γöé       Γö£ΓöÇΓöÇ y_attacks.npy        # integer-encoded attack type labels
Γöé       Γö£ΓöÇΓöÇ y_attacks_str.npy    # string attack type labels
Γöé       Γö£ΓöÇΓöÇ scaler.pkl           # fitted MinMaxScaler (also in models/)
Γöé       Γö£ΓöÇΓöÇ feature_names.json   # ordered feature column list
Γöé       Γö£ΓöÇΓöÇ attack_label_map.json
Γöé       Γö£ΓöÇΓöÇ attack_type_label_map.json  # 11-class map for Attack-Type NN
Γöé       Γö£ΓöÇΓöÇ X_train_attacktype.npy      # attack-type NN training split
Γöé       Γö£ΓöÇΓöÇ X_val_attacktype.npy
Γöé       Γö£ΓöÇΓöÇ X_test_attacktype.npy
Γöé       Γö£ΓöÇΓöÇ y_train_attacktype.npy
Γöé       Γö£ΓöÇΓöÇ y_val_attacktype.npy
Γöé       Γö£ΓöÇΓöÇ y_test_attacktype.npy
Γöé       Γö£ΓöÇΓöÇ sample_weights_train_attacktype.npy  # balanced class weights
Γöé       Γö£ΓöÇΓöÇ dqn_states.npy               # full state vectors (115+1+N+1 dims)
Γöé       Γö£ΓöÇΓöÇ dqn_optimal_actions.npy      # ground-truth action labels (reward only)
Γöé       Γö£ΓöÇΓöÇ dqn_labels_str.npy           # string attack labels for per-class eval
Γöé       Γö£ΓöÇΓöÇ dqn_train_*.npy / dqn_val_*.npy / dqn_test_*.npy  # DQN splits
Γöé       ΓööΓöÇΓöÇ attack_class_counts.json
Γö£ΓöÇΓöÇ simulator/               # CICIDS2017 replay simulator (demo infrastructure)
Γöé   Γö£ΓöÇΓöÇ replay_simulator.py  # streams CSV rows to /predict as if live traffic
Γöé   Γö£ΓöÇΓöÇ streamlit_app.py     # interactive Streamlit UI to manually inject specific
Γöé   Γöé                        #   attack types into the backend for testing/debugging
Γöé   ΓööΓöÇΓöÇ README.md            # usage: --rate, --benign-ratio, --host flags
Γö£ΓöÇΓöÇ notebooks/               # Jupyter notebooks for EDA
Γö£ΓöÇΓöÇ training/
Γöé   Γö£ΓöÇΓöÇ preprocess_benign.py          # Step 1 ΓÇö benign data ΓåÆ autoencoder training data
Γöé   Γö£ΓöÇΓöÇ preprocess_attack.py          # Step 2 ΓÇö attack data ΓåÆ DQN environment data
Γöé   Γö£ΓöÇΓöÇ train_autoencoder.py          # Stage 1A ΓÇö autoencoder training
Γöé   Γö£ΓöÇΓöÇ build_hybrid_dataset.py       # Stage 1B data prep ΓÇö 7-class hybrid dataset
Γöé   Γö£ΓöÇΓöÇ train_hybrid_classifier.py    # Stage 1B training ΓÇö XGBoost hybrid classifier
Γöé   Γö£ΓöÇΓöÇ build_attack_type_dataset.py  # Stage 2 data prep ΓÇö 11-class attack-type dataset
Γöé   Γö£ΓöÇΓöÇ train_attack_type_nn.py       # Stage 2 training ΓÇö feedforward attack-type NN
Γöé   Γö£ΓöÇΓöÇ build_dqn_environment.py      # Stage 3 data prep ΓÇö builds full DQN state vectors
Γöé   Γöé                                 #   (runs AE + Attack-Type NN forward passes)
Γöé   Γö£ΓöÇΓöÇ dqn_env.py                    # Stage 3 Gymnasium env ΓÇö RemediationEnv (single-step)
Γöé   ΓööΓöÇΓöÇ train_dqn.py                  # Stage 3 training ΓÇö DQN consumes [features | AE error
Γöé                                     #   | attack-type NN softmax probs | confidence]
Γö£ΓöÇΓöÇ models/                  # saved artefacts (output of training)
Γöé   Γö£ΓöÇΓöÇ autoencoder.pt          # ~90 KB  (state_dict, 115ΓåÆ80ΓåÆ48ΓåÆ24ΓåÆ48ΓåÆ80ΓåÆ115)
Γöé   Γö£ΓöÇΓöÇ hybrid_classifier.pkl   # ~2.5 MB (XGBoost, 7-class)
Γöé   Γö£ΓöÇΓöÇ hybrid_label_encoder.pkl
Γöé   Γö£ΓöÇΓöÇ attack_type_nn.pt       # feedforward NN (115ΓåÆ128ΓåÆ64ΓåÆ11)
Γöé   Γö£ΓöÇΓöÇ attack_type_label_map.json  # int ΓåÆ class-name mapping
Γöé   Γö£ΓöÇΓöÇ attack_type_nn_history.json # train/val loss per epoch
Γöé   Γö£ΓöÇΓöÇ dqn_agent.pt            # ~95 KB  (state_dimΓåÆ128ΓåÆ64ΓåÆ5)
Γöé   Γö£ΓöÇΓöÇ dqn_training_history.json   # reward curve over episodes
Γöé   Γö£ΓöÇΓöÇ scaler.pkl              # ~7 KB   (copy from data/processed/)
Γöé   Γö£ΓöÇΓöÇ threshold.json          # <1 KB   (95th-percentile val MSE)
Γöé   ΓööΓöÇΓöÇ training_history.json   # autoencoder train/val loss per epoch
Γö£ΓöÇΓöÇ backend/
Γöé   Γö£ΓöÇΓöÇ main.py              # FastAPI app + WebSocket
Γöé   Γö£ΓöÇΓöÇ inference.py         # model inference logic
Γöé   Γö£ΓöÇΓöÇ action_executor.py   # dummy action executor (simulated network state)
Γöé   Γö£ΓöÇΓöÇ database.py          # SQLAlchemy + PostgreSQL
Γöé   ΓööΓöÇΓöÇ schemas.py           # Pydantic models
Γö£ΓöÇΓöÇ frontend/
Γöé   ΓööΓöÇΓöÇ incident-dashboard/  # Vite+React 19+TypeScript+Tailwind v4
Γöé       Γö£ΓöÇΓöÇ src/
Γöé       Γöé   Γö£ΓöÇΓöÇ App.tsx          # ThreatSentinel dashboard UI
Γöé       Γöé   Γö£ΓöÇΓöÇ App.css          # component styles
Γöé       Γöé   Γö£ΓöÇΓöÇ index.css        # design system + Tailwind
Γöé       Γöé   ΓööΓöÇΓöÇ main.tsx         # React entry point
Γöé       Γö£ΓöÇΓöÇ index.html
Γöé       Γö£ΓöÇΓöÇ vite.config.ts
Γöé       ΓööΓöÇΓöÇ package.json
Γö£ΓöÇΓöÇ docs/
Γöé   Γö£ΓöÇΓöÇ architecture_document.docx
Γöé   ΓööΓöÇΓöÇ project_charter.docx
...
#    --rate: flows per second | --benign-ratio: fraction of benign rows (0.0ΓÇô1.0)
...
#     into the backend ΓÇö useful for testing and debugging specific attack classes
...
*README last updated: 30 June 2026 ΓÇö Added interactive Streamlit simulator frontend (`simulator/streamlit_app.py`): manually inject specific attack types into the FastAPI backend for targeted testing and debugging.*
